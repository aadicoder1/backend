...
@router.post("/delete/{doc_id}")  # fallback if DELETE blocked
def delete_document(doc_id: int, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        # ✅ Ownership check only if model actually has 'uploaded_by'
        if hasattr(doc, "uploaded_by"):
            if (doc.uploaded_by or "").lower() != (current_user.username or "").lower():
                raise HTTPException(
                    status_code=403,
